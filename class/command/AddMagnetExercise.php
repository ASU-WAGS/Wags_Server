...
class AddMagnetExercise extends Command
{
    ...
    {
        $title = $_POST['title'];
        //Makes sure the title is not null
        if ($title == null) {
            return JSON::error("Please enter a title");
        }
        $desc = $_POST['desc'];
        $type = $_POST['type'];
        ...
        if ($_FILES['testClass']['size'] != 0) {
            ...
            $result = $this->addSimpleFile($_FILES['testClass'], 1, $title);
            if ($result != 1) {
                return JSON::error("Test Class: " . $result);
            }
            $hasFiles = true;
        }
    }
        ...
        if (($_FILES[$helperName]['size']) != 0) {

            while ($helperId < count($_FILES) && $_FILES[$helperName]['size'] != 0) {

                $result = $this->addSimpleFile($_FILES[$helperName], 0, $title);
                if ($result != 1) {

                    return JSON::error("Helper Class: " . $result);
                }
                $helperId++;
    ...
    function addSimpleFile($file, $testValue,$title) {
        $fileName = pathinfo($file["name"], PATHINFO_FILENAME);
        $fileExtension = pathinfo($file["name"], PATHINFO_EXTENSION);

        //Queries the database for the id of the parent problem using the title
        $db = Database::getDb();
        $sth = $db -> prepare('SELECT id FROM magnetProblem WHERE title = ?');
        ...
        $newSF->setMagnetProblemId($result->id); // 'holding' for linkage
